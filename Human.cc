...
int Human::move()
{
    ...
    while (getWords(words))
    {
        ...
        else if (words.size() == 2 && words[0] == "setlevel" 
			&& words[1].length() == 1)
        {
	...
	// invalid coord
        else if (!coordToIdx(words[1], orig) || !coordToIdx(words[2], dest))
        {
        ...
        else if (!idx.count(orig)) // no piece or not my piece
        {
            cout << "Error: No such a piece!" << endl;
                 ...
                 !(enPassantMove.size() && enPassantMove[0] == orig &&
			 enPassantMove[1] == dest)) // cannot validly move it
        {
...
// setLevel(level) sets the difficulty level
